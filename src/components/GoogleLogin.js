...
export default function GoogleLogin({ onSuccess, onError, className = "" }) {
  ...
  return (
    <button
      onClick={handleLogin}
      className={`w-full flex items-center justify-center gap-3 px-4 py-3 border border-gray-300 rounded-md hover:bg-gray-50 transition-colors cursor-pointer ${className}`}
    >
      <svg
        width="18"
        height="18"
